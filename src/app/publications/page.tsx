...
const PublicationsPage = () => {
  ...
  const years = publications
    .map(pub => pub.year)
    .reduce((acc, year) => acc.includes(year) ? acc : [...acc, year], [] as string[])
              ...
              <div className={`flex-1 ${pub.image ? '' : 'md:ml-0'}`}>
                <div className="flex items-center mb-2">
                  <h2 className="text-lg font-bold text-gray-800">
                    ...
                    {pub.title[locale as keyof typeof pub.title]}
                  </h2>
                </div>
                <p className="text-gray-700 mb-2">
                  {pub.authors[locale as keyof typeof pub.authors]}
                </p>
                <p className="text-gray-700 mb-2">
                  <span className="italic">{pub.journal[locale as keyof typeof pub.journal]}</span>, 
